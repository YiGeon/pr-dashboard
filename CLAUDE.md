...
- `src/lib/stores/filters.ts` — 필터/정렬/검색/탭/포커스/그룹핑 상태 (groupByRepo는 localStorage 영속), highlightedPRId (3초 자동 해제)
...
- `src/lib/notifications.ts` — 이전/현재 데이터 diff → 브라우저 알림 트리거 + 알림 클릭→카드 스크롤/하이라이트
...
│   │   │   ├── FilterBar.svelte    #   Org 필터, 정렬, 검색, Flat/Grouped Segmented Control
│   │   │   ├── Login.svelte        #   GitHub 로그인 화면
│   │   │   ├── PRCard.svelte       #   PR 카드 (클릭→GitHub + ▸확장/프리뷰 + entity 컬러 뱃지 + 알림 하이라이트)
...
│   │   │   ├── filters.ts          #   필터/정렬/검색/탭/포커스/그룹핑/하이라이트 + 로직
│   │   │   ├── prs.ts              #   PR 데이터 + 폴링 + derived (urgent, updateCount)
│   │   │   └── settings.ts         #   설정 영속 저장 (localStorage)
│   │   ├── notifications.ts        #   알림 diff 감지 + Notification API + 알림→카드 내비게이션
...
- Flat/Grouped 뷰 전환은 Segmented Control (라디오 버튼 방식) — FilterBar 내 `view-toggle` div
- 알림 클릭 시 `navigateToNotification`은 탭 전환 + 필터 초기화 + `highlightedPRId` 설정 → PRCard에서 `scrollIntoView` + border glow 애니메이션 (3초). `prId` 없는 기존 알림은 무시 (fallback 없음)
- `AppNotification`에 `prId` 필드 필수 — `addNotification` 호출 시 반드시 포함. localStorage의 기존 알림에 `prId`가 없을 수 있으나 호환 처리하지 않음
