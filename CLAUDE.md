...
- `src/lib/stores/filters.ts` — 필터/정렬/검색/탭/포커스 상태 (focusedIndex)
- `src/lib/utils.ts` — 유틸 함수 (relativeTime, formatDate, labelTextColor, STATUS_COLORS/ICONS/LABELS, formatNotificationBody)
...
│   │   │   ├── FilterBar.svelte    #   Org 필터, 정렬, 검색 (외부 클릭 닫기)
│   │   │   ├── Login.svelte        #   GitHub 로그인 화면
│   │   │   ├── PRCard.svelte       #   PR 카드 (상태바 + 리뷰어 + 머지/라벨/diff + 키보드 포커스)
│   │   │   ├── PRList.svelte       #   PR 카드 리스트 (스켈레톤 로딩 + 빈 상태)
...
│   │   │   ├── filters.ts          #   필터/정렬/검색/탭/포커스 상태 + 로직
...
│   │   └── utils.ts                #   유틸 함수 (상대시간, 날짜포맷, 라벨색상, STATUS 상수 등)
│   ├── app.css                     # 글로벌 스타일
│   └── app.html                    # HTML 템플릿
├── tests/                          # Vitest 테스트
│   ├── lib/
