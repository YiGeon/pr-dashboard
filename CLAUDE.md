...
- `src/lib/stores/filters.ts` — 필터/정렬/검색/탭/포커스/그룹핑/아카이브 상태
- `src/lib/stores/notes.ts` — PR별 메모/우선순위 태그 (localStorage 영속)
- `src/lib/stores/archive.ts` — PR 아카이브 (localStorage 영속)
...
│   │   │   ├── FilterBar.svelte    #   Org 필터, 정렬, 검색, 그룹핑, 아카이브 토글
│   │   │   ├── Login.svelte        #   GitHub 로그인 화면
│   │   │   ├── PRCard.svelte       #   PR 카드 (확장/프리뷰 + 메모/태깅 + 아카이브 + entity 컬러 뱃지)
...
│   │   │   ├── filters.ts          #   필터/정렬/검색/탭/포커스/그룹핑/아카이브 + 로직
│   │   │   ├── notes.ts            #   PR별 메모/우선순위 (localStorage)
│   │   │   ├── archive.ts          #   PR 아카이브 (localStorage)
...
- PR 카드 클릭 시 확장/축소 토글 — GitHub 링크는 ↗ 버튼으로 분리. 확장 시 커밋/코멘트 on-demand fetch (`PR_DETAIL_QUERY`)
- PR 메모/우선순위/아카이브는 `localStorage`에 PR ID 기반 저장 — PR이 닫히면 데이터가 남지만 자동 정리하지 않음
